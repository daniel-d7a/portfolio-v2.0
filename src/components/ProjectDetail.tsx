        ...
        <header className="mb-16 md:mb-24">
          ...
          >
            ...
            <h1 className="text-5xl md:text-8xl font-display font-black tracking-tighter leading-[0.9] mb-8">
              {project.name}<span className="ms-1 text-brand-tertiary">.</span>
            </h1>
        ...
        >
          <div className="md:col-span-7">
            {project.images.map((image) => (
              <div className="overflow-hidden rounded-2xl bg-brand-fg/5 border border-brand-border mb-12">
                <img
                  src={image}
                  alt={project.name}
            ...
            <div className="sticky top-32 space-y-12">
              <div className="p-8 rounded-2xl border border-brand-border bg-brand-fg/5 backdrop-blur-sm">
                ...
                <dl className="space-y-6">
                  <div>
                    <dt className="text-xs opacity-50 font-bold tracking-widest uppercase mb-1">Type</dt>
                    <dd className="text-sm font-medium">{project.type}</dd>
                  </div>
                  <div>
                    <dt className="text-xs opacity-50 font-bold tracking-widest uppercase mb-1">Tech Stack</dt>
                    <dd className="text-sm font-medium">{project.stack.join(', ')}</dd>
                  </div>
                </dl>
                {project.livePreviewLink &&
                  <a href={`${project.livePreviewLink}`} target='_blank'  className="w-full mt-10 py-4 px-6 bg-brand-fg text-brand-bg font-display font-bold text-sm tracking-widest uppercase rounded-lg hover:opacity-90 transition-opacity flex items-center justify-center gap-2 group">
